...
if (PRODUCT_LIST_ONMOUSEOVER_BUTTONS == "true") {
	...
	$add_button_buynow = '<br>'.$button_buynow;
	  ...
	  $info_box_contents = '<li><span class="smallText"><div class="pl_style_border_div">'.$add_categories_subcategories.'<span class="pl_expire_date">'.$default_specials_expiredate.'</span>'.$products_image_list.$add_likes_box.''.$button_free_shipping.'<br><br>'.$button_youtube.'<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $random_product['products_id']) . '">' . $manufacturers_name . $products_model . '<span class="pl_product_name">' . $random_product['products_name'] . '</span></a>'.$unidade_venda.'<br>'.$price_product.$add_button_buynow.'</div></span></li>';
