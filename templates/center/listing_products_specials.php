...
if (ENABLE_YOUTUBE_PRODUCTLISTING == 'true') {
	if (!empty($specials['products_youtube'])) {
	  if (strstr($specials['products_youtube'], "<iframe")) {
		...
		$button_youtube = '<a href="'.str_replace("watch?v=", "embed/", $specials['products_youtube']).'" class="fancybox fancybox.iframe">'.tep_image(DIR_WS_IMAGES . 'icon_youtube_play.png', '', '', '', ' align="middle" ').'</a>';
	  }
	}else{
		$button_youtube = '';
	}
...
onmouseout="document.getElementById(\'div_name'.$specials['products_id'].'\').style.display=\'none\';">'.$add_categories_subcategories.'<span class="pl_expire_date">'.$specials_expiredate.'</span>'.$products_image_list.$add_likes_box.''.$button_free_shipping.'<br>'.$button_youtube.'<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $specials['products_id']) . '" itemprop="url">'.$manufacturers_name.$products_model.'<span class="pl_product_name">' . $specials['products_name'] . '</span></a>'.$unidade_venda.'<br>'.$price_product.$add_button_buynow.'</div></td>' . "\n";
