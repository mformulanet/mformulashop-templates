...
// button free shipping eof
		...
		if ( (VIEW_BUTTON_NEW == 'true') ||(VIEW_BUTTON_NEW == 'True') ) {
			$products_date_added = explode(" ", $random_product['products_date_added']);	
		...
		if ( (VIEW_BUTTON_UPDATED == 'true') ||(VIEW_BUTTON_UPDATED == 'True') ) {
			if (!empty($random_product['products_last_modified'])) {
			$products_last_modified = explode(" ", $random_product['products_last_modified']);
...
onmouseout="document.getElementById(\'div_name'.$random_product['products_id'].'\').style.display=\'none\';"><a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $random_product['products_id']) . '"><center>' . $products_image_list . '</center></a>'.$add_likes_box.$button_free_shipping.'<br><br>'.$button_youtube.'<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $random_product['products_id']) . '">' . $manufacturers_name . $products_model .'<span class="pl_product_name">' . $random_product['products_name'] . '</span></a>'.$unidade_venda.'<br>'.$whats_new_price.$button_new.$button_update.'<span class="pl_expire_date">'.$specials_expiredate.'</span>'.$add_button_buynow.'</div>');
