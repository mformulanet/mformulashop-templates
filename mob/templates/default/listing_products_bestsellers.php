...
$product_check_query = tep_db_query("select status_sell_customer_group_discount from " . TABLE_PRODUCTS . " where products_id = '" . (int)$random_product['products_id'] . "'");
...
if ( ($random_product['products_quantity'] > 0) || ( ($random_product['products_quantity'] <= 0) && ($random_product['products_qtd_stock_status'] == '1') ) ) {
    ...
    if (tep_not_null($random_product['specials_new_products_price'])) {
	  ...
	  if (($random_product['parcel_window'] == 1) && ($language == 'portugues') && ($currency == 'BRL')) {
		...
		$lc_text = '<br>'.TEXT_PRICE_PRODUCT_FROM.'<s>' . $currencies->display_price($price, tep_get_tax_rate($random_product['products_tax_class_id'])) . '</s>';
		
		$lc_text .= '<br><span class="pl_product_special_price"><b>' . TEXT_PRICE_PRODUCT_TO . $currencies->display_price($newprice, tep_get_tax_rate($random_product['products_tax_class_id'])) . '</b></span>';
		...
		$lc_text .= '<br>'.TEXT_PRICE_PRODUCT_OR.'<span class="pl_product_special_price"><b>' . $currencies->display_price($new_products_price_ok, tep_get_tax_rate($random_product['products_tax_class_id'])) . '</b></span>' . ' ' . MODULE_PAYMENT_DISC_PERCENTAGE . '% ' . TEXT_OF_DISCOUNT . TEXT_PRICE_PRODUCT_AVISTA;
		...
		$lc_text .= '<br>'.TEXT_PRICE_PRODUCT_OR.'<span class="pl_product_special_price"><b>' . $currencies->display_price($new_products_price_ok, tep_get_tax_rate($random_product['products_tax_class_id'])) . '</b></span>' . ' ' . MODULE_PAYMENT_DISC1_PERCENTAGE . '% ' . TEXT_OF_DISCOUNT . TEXT_PRICE_PRODUCT_AVISTA;

		}
		
		if ( (NUMBER_PLOTS_VISA > NUMBER_PLOTS_JUROS) && (VIEW_PLOTS_JUROS == 'true') ) {
		
		$lc_text .= '<br>'.TEXT_PRICE_PRODUCT_OR.'<span class="pl_product_plots_price"><b>'.NUMBER_PLOTS_VISA.'x ' . $price_divide . '</b></span>' . ' ' . TEXT_COMJUROS . ' ' . TAXA_PLOTS_JUROS . '% a.m.';
		
		}else{
		
		$lc_text .= '<br>'.TEXT_PRICE_PRODUCT_OR.'<span class="pl_product_plots_price"><b>'.NUMBER_PLOTS_VISA.'x ' . $price_divide . '</b></span>' . ' ' . TEXT_SEMJUROS;
		...
		$lc_text = '<br><s>' . $currencies->display_price($price, tep_get_tax_rate($random_product['products_tax_class_id'])) . '</s><br>';
		$lc_text .= '<strong><span class="pl_product_special_price">' . $currencies->display_price($newprice, tep_get_tax_rate($random_product['products_tax_class_id'])) . '<br>'.NUMBER_PLOTS_VISA.'x ' . $price_divide . '</span></strong>';
	  ...
	  $lc_text = '<br><s>' . $currencies->display_price($price, tep_get_tax_rate($random_product['products_tax_class_id'])) . '</s><br>';
      $lc_text .= '<strong><span class="pl_product_special_price">' . $currencies->display_price($newprice, tep_get_tax_rate($random_product['products_tax_class_id'])) . '</span></strong>';
	  ...
	  if (($random_product['parcel_window'] == 1) && ($language == 'portugues') && ($currency == 'BRL')) {
		...
		$lc_text .= '<br>'.TEXT_PRICE_PRODUCT_FROM."<s>" . $currencies->display_price($new_price_add, tep_get_tax_rate($random_product['products_tax_class_id'])) . "</s>";
		}
		}
		// fake special price

		if ($discount > 0) {
		$lc_text .= '<br>'.TEXT_PRICE_PRODUCT_FROM.'<s>' . $currencies->display_price($orig_price, tep_get_tax_rate($random_product['products_tax_class_id'])) . '</s>';
		}
				
		$lc_text .= '<br><span class="pl_product_price"><b>' . TEXT_PRICE_PRODUCT_TO . $currencies->display_price($prod_price, tep_get_tax_rate($random_product['products_tax_class_id'])) . '</b></span>';
		...
		if ($discount > 0) {
		$lc_text .= '<br><s>'.$currencies->display_price($orig_price, tep_get_tax_rate($random_product['products_tax_class_id'])) . '</s>';
		}
		
		$lc_text .= '<br><strong><span class="pl_product_price">'.$currencies->display_price($prod_price, tep_get_tax_rate($random_product['products_tax_class_id'])) . '</span><br><span class="pl_product_plots_price"> '.NUMBER_PLOTS_VISA.'x ' . $price_divide . '</span></strong>';
		...
		$lc_text .= '<br><strong><span class="pl_product_price">'.$currencies->display_price($price, tep_get_tax_rate($random_product['products_tax_class_id'])) . '</span></strong>';
...
// button youtube play eof
	  ...
	  $info_box_contents[$row][$col] = array('align' => 'center',
                                          ...
                                          'text' => '<div class="pl_style_border_div">
										   ...
										   <br>'.$button_youtube.'<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $random_product['products_id']) . '">' . $manufacturers_name . $products_model . '<span class="pl_product_name">' . $random_product['products_name'] . '</span></a>'.$unidade_venda.'<br>'.$lc_text.'
...
onmouseout="document.getElementById(\'div_name'.$random_product['products_id'].'\').style.display=\'none\';">'.$add_categories_subcategories.$products_image_list.''.$button_free_shipping.'<br><br>'.$button_youtube.'<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $random_product['products_id']) . '">' . $manufacturers_name . $products_model . '<span class="pl_product_name">' . $random_product['products_name'] . '</span></a>'.$unidade_venda.'<br>'.$lc_text.'<span class="pl_expire_date">'.$specials_expiredate.'</span><br>'.$button_buynow.'</div>');
