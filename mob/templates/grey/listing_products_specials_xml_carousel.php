...
// expire date and time
	  if ( ($products_quantity > 0) && (DISPLAY_EXPIRE_DATE_SPECIALS == 'true') ) {
		  if ( (tep_not_null($expires_date)) && ($expires_date != '0000-00-00 00:00:00')) {
			  ...
			  $date_expire_replace = str_replace(array_keys($replacements), $replacements, calculate_date_between(date("Ymd H:i:s"), str_replace($replace_date_expire, "", $expires_date)));
			  
			  $specials_expiredate = TEXT_SPECIALS_EXPIREDATE . ' ' . tep_date_short($expires_date) . '<br>' . TEXT_SPECIALS_EXPIREDATE_FALTA . ' ' . $date_expire_replace . '<br>';
		  }else{
			  $specials_expiredate = '';
		  }
	  }
...
// price and parcel
		$newprice = tep_get_products_special_price($products_id);
		...
		if ($discount > 0) {
		$price = tep_get_products_special_price($products_id);
		}else{
		...
		}elseif ( ($products_quantity > 0) || ( ($products_quantity <= 0) && ($random_product['products_qtd_stock_status'] == '1') ) ) {
	
		if (($parcel_window == 1) && ($language == 'portugues') && ($currency == 'BRL')) {
		
		if (TYPE_PRODUCT_PRICE_VIEW == 'COM-AVISTAPARCELA') {
		
		$price_product = '<br>'.TEXT_PRICE_PRODUCT_FROM.'<s>' . $currencies->display_price($price, tep_get_tax_rate($products_tax_class_id)) . '</s>';
		
		$price_product .= '<br><span class="pl_product_special_price"><b>' . TEXT_PRICE_PRODUCT_TO . $currencies->display_price($newprice, tep_get_tax_rate($products_tax_class_id)) . '</b></span>';
			...
			$price_product .= '<br>'.TEXT_PRICE_PRODUCT_OR.'<span class="pl_product_special_price"><b>' . $currencies->display_price($new_products_price_ok, tep_get_tax_rate($products_tax_class_id)) . '</b></span>' . ' ' . MODULE_PAYMENT_DISC_PERCENTAGE . '% ' . TEXT_OF_DISCOUNT . TEXT_PRICE_PRODUCT_AVISTA;
			...
			$price_product .= '<br>'.TEXT_PRICE_PRODUCT_OR.'<span class="pl_product_special_price"><b>' . $currencies->display_price($new_products_price_ok, tep_get_tax_rate($products_tax_class_id)) . '</b></span>' . ' ' . MODULE_PAYMENT_DISC1_PERCENTAGE . '% ' . TEXT_OF_DISCOUNT . TEXT_PRICE_PRODUCT_AVISTA;
		...
		$price_product = '<br><s>' . $currencies->display_price($price, tep_get_tax_rate($products_tax_class_id)) . '</s><br><span class="pl_product_special_price"><b>' . $currencies->display_price($newprice, tep_get_tax_rate($products_tax_class_id)) . '</b></span><br><span class="pl_product_plots_price"><strong>'.NUMBER_PLOTS_VISA.'x ' . $price_divide . '</strong></span>';
		
		}
		
		}else{
		
		$price_product = '<br><s>' . $currencies->display_price($price, tep_get_tax_rate($products_tax_class_id)) . '</s><br><span class="pl_product_special_price"><b>' . $currencies->display_price($newprice, tep_get_tax_rate($products_tax_class_id)) . '</b></span>';
...
$add_ce = '</div>';
	...
	$info_box_contents = '
										   '.$add_cs.'<span class="smallText"><div class="pl_style_border_div">
										   ...
										   <br><a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $products_id) . '" itemprop="url">'.$manufacturers_name.$products_model.'<span class="pl_product_name">' . $products_name . '</span></a> '.$unidade_venda.$price_product.'
	...
	$info_box_contents[$row][$col] = array('align' => 'center',
										   ...
										   '.$add_categories_subcategories.'
										   <span class="pl_expire_date">'.$specials_expiredate.'</span>
										   <br>'.$button_youtube.'<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $products_id) . '" itemprop="url">'.$manufacturers_name.$products_model.'<span class="pl_product_name">' . $products_name . '</span></a> '.$unidade_venda.$price_product.'
...
// expire date and time
	  if ( ($products_quantity > 0) && (DISPLAY_EXPIRE_DATE_SPECIALS == 'true') ) {
		  if ( (tep_not_null($expires_date)) && ($expires_date != '0000-00-00 00:00:00')) {
			  ...
			  $specials_expiredate = TEXT_SPECIALS_EXPIREDATE . ' ' . tep_date_short($expires_date) . '<br>' . TEXT_SPECIALS_EXPIREDATE_FALTA . ' ' . $date_expire_replace . '<br><br>';
		  }else{
			  $specials_expiredate = '';
		  }
	  }
...
$add_ce = '</div>';
	...
	$info_box_contents = $add_cs.'<span class="smallText"><div class="pl_style_border_div">'.$add_categories_subcategories.'<span class="pl_expire_date">'.$specials_expiredate.'</span>'.$products_image_list.$add_likes_box.''.$button_free_shipping.'<br><br>'.$button_youtube.'<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $products_id) . '" itemprop="url">'.$manufacturers_name.$products_model.'<span class="pl_product_name">' . $products_name . '</span></a>'.$unidade_venda.'<br>'.$price_product.$button_buynow.'</div></span>'.$add_ce;
...
onmouseout="document.getElementById(\'div_name'.$products_id.'\').style.display=\'none\';">'.$add_categories_subcategories.'<span class="pl_expire_date">'.$specials_expiredate.'</span>'.$products_image_list.$add_likes_box.''.$button_free_shipping.'<br><br>'.$button_youtube.'<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $products_id) . '" itemprop="url">'.$manufacturers_name.$products_model.'<span class="pl_product_name">' . $products_name . '</span></a>'.$unidade_venda.'<br>'.$price_product.'<br>'.$button_buynow.'</div>');
