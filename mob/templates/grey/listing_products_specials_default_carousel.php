...
onmouseout="document.getElementById(\'div_name'.$random_product['products_id'].'\').style.display=\'none\';">'.$add_categories_subcategories.'<span class="pl_expire_date">'.$default_specials_expiredate.'</span>'.$products_image_list.$add_likes_box.''.$button_free_shipping.'<br><br>'.$button_youtube.'<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $random_product['products_id']) . '">' . $manufacturers_name . '<span class="pl_product_name">' . $random_product['products_name'] . '</span></a>'.$unidade_venda.'<br>'.$price_product.'<br>'.$button_buynow.'</div></span>'.$add_ce;
